...
import io.waylay.kairosdb.driver.models.TimeRange._
...
object Formats {
  ...
  implicit val datapointWrites: Writes[DataPoint] = new Writes[DataPoint] {
    ...
    implicit val datapointWithTimeStampWrites: Writes[(Instant, KairosCompatibleType)] =
      (point: (Instant, KairosCompatibleType)) => {
  ...
  implicit def groupByWrites[T <: GroupBy]: Writes[T] = (groupBy: T) => {
      ...
      val valueRes = (json \ "value").validate[Long].orElse {
        (json \ "value").validate[String].flatMap { s =>
          Try(s.toLong) match {
            ...
            case Failure(_) => JsError("error.expected.jsnumber")
          }
        }
      }
  ...
  implicit val groupByReads: Reads[GroupBy] = (json: JsValue) => {
  ...
  implicit val samplerAggregatorWrites: Writes[Sampler] = (sampler: Sampler) => {
    val base = Json.obj(
      "name" -> sampler.name,
  ...
  implicit val rateAggregatorWrites: Writes[Rate] = (rate: Rate) => {
    val base = Json.obj(
      "name" -> rate.name,
  ...
  implicit val rangeAggregatorWrites: Writes[RangeAggregator] = (rangeAgg: RangeAggregator) => {
    Json.obj(
      "name" -> rangeAgg.name,
  ...
  implicit val percentileAggregatorWrites: Writes[Percentile] = (percentileAgg: Percentile) => {
    Json.obj(
  ...
  implicit def timePointWrites[T <: TimePoint]: Writes[T] = {
  ...
  implicit val orderWrites: Writes[Order] = (order: Order) => Json.toJson(order.value)

  implicit val queryTagFormat: OFormat[QueryTag] = Json.format[QueryTag]
  ...
  implicit val queryWrites: Writes[Query] = (query: Query) => {
  ...
  implicit val metricNameAsStringReads: Reads[MetricName] =
    (json: JsValue) => json.validate[String].map(MetricName)
  ...
  implicit val tagsResultReads: Reads[TagsResult] = (
    (JsPath \ "name").read[MetricName] and
      (JsPath \ "tags").read[Seq[TagResult]]
    )(TagsResult.apply _)

  implicit val responseTagsReads: Reads[TagsResponse] = Json.reads[TagsResponse]
  implicit val tagResponseReads: Reads[TagQueryResponse] = Json.reads[TagQueryResponse]

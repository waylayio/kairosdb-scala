...
class DeleteDataPointsByTagIntegrationSpec extends IntegrationSpec {
  "Inserting, deleting and then querying data points" should "only return data points that weren't matched by delete" in {
    ...
    res should be(QueryResponse.Response(Seq(ResponseQuery(3, Seq(
      Result(
        ...
        Seq(TagResult("htns", Seq("888", "999")), TagResult("snth", Seq("321"))), // not sure if the order is deteministic. Convert to Set?
        Seq(
